    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        dispatcher.register(Commands.literal("punishments")
                ...
                .executes(context -> {
                    ...
                    if (punishments.isEmpty()) {
                        context.getSource().sendSuccess(
                                () -> ModMessages.get(context.getSource(),
                                        "command.mod_de_teste.punishments.none",
                    ...
                    context.getSource().sendSuccess(
                            () -> ModMessages.get(context.getSource(),
                                    "command.mod_de_teste.punishments.header",
                                    "Active punishments:"
                            ), false
                    );
                    ...
                    for (PunishmentRegistry.PunishmentData data : punishments.values()) {
                        ...
                        context.getSource().sendSuccess(
                                () -> ModMessages.get(context.getSource(),
                                        "command.mod_de_teste.punishments.player",
                                        playerName + ":",
                                        playerName
                                ), false
                        );
                        ...
                        if (data.freezeExpiration != null) {
                            ...
                            context.getSource().sendSuccess(
                                    () -> ModMessages.get(context.getSource(),
                                            "command.mod_de_teste.punishments.freeze",
                                            "- Freeze: " + remaining,
                                            remaining
                                    ), false
                            );
                        ...
                        if (data.muteExpiration != null) {
                            ...
                            context.getSource().sendSuccess(
                                    () -> ModMessages.get(context.getSource(),
                                            "command.mod_de_teste.punishments.mute",
                                            "- Mute: " + remaining,
                                            remaining
                                    ), false
                            );
                ...
                .then(Commands.argument("player", StringArgumentType.word())
                        .executes(context -> {
                            ...
                            if (data == null || data.freezeExpiration == null) {
                                context.getSource().sendSuccess(
                                        () -> ModMessages.get(context.getSource(),
                                                "command.mod_de_teste.checkfreeze.none",
                                                playerName
                                                        + " is not frozen.",
                                                playerName
                                        ), false
                            ...
                            context.getSource().sendSuccess(
                                    () -> ModMessages.get(context.getSource(),
                                            "command.mod_de_teste.checkfreeze.active",
                                            playerName
                                                    + " is frozen. Remaining: "
                                                    + remaining,
                                            playerName,
                ...
                .then(Commands.argument("player", StringArgumentType.word())
                        .executes(context -> {
                            ...
                            if (data == null || data.muteExpiration == null) {
                                context.getSource().sendSuccess(
                                        () -> ModMessages.get(context.getSource(),
                                                "command.mod_de_teste.checkmute.none",
                                                playerName
                                                        + " is not muted.",
                                                playerName
                                        ), false
                            ...
                            context.getSource().sendSuccess(
                                    () -> ModMessages.get(context.getSource(),
                                            "command.mod_de_teste.checkmute.active",
                                            playerName
                                                    + " is muted. Remaining: "
                                                    + remaining,
                                            playerName,
    ...
    private static String formatExpiration(CommandSourceStack source, long expiration) {
        if (expiration == 0L) {
            return ModMessages.get(
                    source,
                    "command.mod_de_teste.punishments.permanent",
                    "Permanent"
            ).getString();
        }
